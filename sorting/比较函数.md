...
## set、map、priority_queue，构造函数参数中的 Compare

set、map 该参数定义为 `class Compare = std::less<Key>`

priority_queue 该参数定义为 `class Compare = std::less<typename Container::value_type>`

是个 class，所以不能用「比较函数」，得用 functor，类似下面的：
...
或者避开这个参数。在 struct 里或外重载 `operator()`，即可。（但若比较指针，则不能用此法）
