...
- [lint 156](https://www.lintcode.com/problem/156)：[c++ 代码](code/lint-156-merge-intervals.cpp)
- [leet 56](https://leetcode.cn/problems/merge-intervals)：[c++ 代码](code/leet-56-merge-intervals.cpp)
- [LCR 074](https://leetcode.cn/problems/SsGoHC)


原序列已经排好序。排序方式：按 start 排。这样的好处是，新来的最多跟一个重叠，不会出现多个。若按 end 排序，则可能出现新来的跟多个重叠的情况。
...
两种情况的 3，都只跟 2 重叠。不会出现 4、5 这种跟 1、2 都重叠的情况（因为 4、5 要跟 1 重叠，必须得 4.begin < 1.end，从而必然 4.begin < 2.start，从而 4 排在 2 前面。）
...
3、4 都是正确地排在 1、2 后面，且跟 1、2 都重叠。

流式合并，每次来个新区间（记为 `new`），如果跟 `ans` 最后一个区间（记为 `last`）有重叠，就把 `new` 合并到 `last` 里；否则将 `new` 作为单独一项放到 `ans` 里，成为 `ans` 的新的 `last`。
