...
promise:
- 封装一个 state。该状态是 mutable 的，可改，即使在异步操作开始后也可改。
- 提供 callback 机制。在异步操作结束时、失败时、进行中，都可 callback。
...
- Java：Java 8 引入了 CompletableFuture。没有严格遵照 Promise 规范的、专门的 Promise 类，但可用 `CompletableFuture` 来实现类似功能。（`CompletableFuture` 的 `thenAccept()` 和 `exceptionally()` 等，提供了 callback 机制）。但二者还是有很多不同。参看[此文档](java-并发-异步编程.md)。
...
java 中没有严格遵照 Promise 规范的、专门的 Promise 类，但可用 `CompletableFuture` 来实现类似功能。但二者不同。
...
## thread APIs: java vs. c++

https://softwareengineering.stackexchange.com/questions/312644/transitioning-from-c-multithreading-to-java-multithreading

These two APIs are almost exactly the same. The difference is
- 缺少 templates 和 操作符重载
- java Thread 对象，需要显式调用 `.start()` 方法。
...
希望把处理线程的逻辑、真正干活的逻辑二者分离。一种方法就是 thread pool。Java 自带了 thread pool。C++ 没有自带的。
