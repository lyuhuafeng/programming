...
void func(int c) {
    ...
    printf("thread %d locked, %s\n", c, (ulock.owns_lock() ? "locked" : "unlocked")); fflush(stdout);
    cv.wait(ulock, [c](){ return c % 3 == num_to_print; }); // wait 开始后，mtx 变为 unlocked 状态
    // wait() 返回后，mtx 是 locked 状态，用来保护对 num_to_print 的写
    printf("%c %d\n", 'A' + c % 3, c); fflush(stdout);
