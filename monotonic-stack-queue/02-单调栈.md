...
<font color=green>一个发现</font>：越到后来，栈的「坡度」越缓。（直观理解，以「递增栈」为例，只要来了一个「低」的，就会把栈里比它高的都弹出去；长此以往，越来越缓。）好像没什么用，但有利于理解单调栈的特性。应用：[`1673.` 最具竞争力的子序列](leet-1673-竞争力子序列.md)。

<font color=green>2024.04.26 更新</font>：新方法，用 LRM tree 也可求 next 或 previous。参见[此文档](LRM-tree-求-next-或-previous.md)。
