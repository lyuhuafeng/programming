...
发现，若用同一种栈，如单调下降栈，可同时得到 next greater 和 previous greater。（2023.12.16 感悟：从这个图看，「新来者」的 previous greater 是 `top`；再多想一步，当「新来者」入栈后，则为：`top` 的 previous greater 是 `top-1`；以此类推，任意时刻，`top-i` 的 previous greater 都是 `top-(i+1)`。另外，栈内个元素一定满足这个递推关系，但形成这个栈的过程中，很多元素已经被弹出栈了，所以栈内元素并不是所有！）
...
以单调上升栈求 next smaller、previous smaller 为例。如图，从左到右，正向遍历。遍历到 i 号元素时，为使其入栈，可能会弹出若干栈顶元素。为行文方便，把栈顶元素记为 t0，栈顶下面元素记为 t1，再往下是 t2，以此类推。
- t0 是栈顶、被弹出：
...
也就是说，
- 每个被弹出的 top，其 next smaller 都是 i
- 每个被弹出的 top，其 prev smaller 都是 该 top 下面
所有该弹出的 top 弹完后，
- i 的 prev smaller 是此时的 top

可见
- 求 next smaller，只有一种方法，就是 (顶法) `ns[top] = i`。这个好理解，i 是目前遍历到的最新元素，目前不可能知道将来谁比他大或小。但以前遍历过的某元素 (`st.top()`) 的「next smaller/greater」有可能是它。
- 求 prev smaller，有两种方法，(新法) `ps[i] = 当前 top` 或 (顶法) `ps[top] = top_1`。

下面代码，`toppi`、`topni` 表示用「顶法」得到的 prev index、next index，`newpi` 表示用「新法」得到的 prev index。最后发现，两种方法得到的 prev index 是一样的。
...
注意到，最后还在栈里的元素，他们都没有 next smaller，可以直接赋值 -1 或 n。则可以结合「顶法」和「新法」。所有的 prev 用「顶法」求，next 用
...
```cpp
    ...
    for (int i = 0; i < n; i++) {
        ...
        newpi[i] = st.empty() ? -1 : st.top(); // 新法 prev
        st.push(i);
    }
```
...
<font color="red">首尾同时加哨兵，还有个坑</font>：若 (1) 两个哨兵值相等，例如，很自然地都用 INT_MIN；且 (2) while 条件中用的是「`>=`」即 `a[st.top()] >= a[i]`，则在 while 循环体中，<font color="red">尾哨兵最终会把首哨兵弹出，导致 `toppi[ti] = st.top()` 这句 segfault</font>。（更有甚者，有时即使栈为空，st.top() 也不会 segfault，就更难发现问题了。）解决方法，是让两个哨兵值不同，保证尾哨兵不能把首哨兵弹出；麻烦。何况还有「元素索引多 1」的问题。<font color="green">结论：只用尾哨兵，不用首哨兵。</font>
