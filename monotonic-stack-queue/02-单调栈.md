...
- <font color="green">注意，对「顶法」，在 while 循环体内，每个被弹出的 top 元素，都可同时得到其 previous 和 next smaller/greater。</font>previous 是 top_1，next 是新来的 i。结合「尾哨兵」技巧，保证栈内每个元素都被弹出，最方便。<font color="red">注意，仍然是不对称的，一边是 greater，另一边就是 greater-or-equal。</font>
...
注意到，最后还在栈里的元素，他们都没有 next smaller，可以直接赋值 -1 或 n。则可以结合「顶法」和「新法」。所有的 prev 用「新法」求，next 用「顶法」求。
