...
通过合并操作，将等价元素归入一个集合中。为合并等价元素，需先查询待合并的两个元素分别属于哪个集合，分属不同集合时，才需要合并。因此，主要操作就是「查询」与「合并」。
...
- `rank[]` 数组，记录每棵树的 rank。<font color="red">注意，`rank[i]` 表示以 i 为 root 的连通分量树的 rank。其他节点无需设置 rank 值。</font>为使树尽量矮，`find(i)` 时要做路径压缩，i 向上途径的各父节点均直接指向 root。
